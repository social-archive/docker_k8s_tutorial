...
1. **지금 하는 일의 의미 설명 (이론 강의)**
   - 각 날짜별 디렉토리에 신규 생성된 `lecture*.md` (이론 강의 교안) 파일을 띄우거나 노션으로 공유하여 이론 교육을 진행한다.
   - **5단계 설명 프레임워크**(`비유 ➡️ 없으면 불편함 ➡️ 왜 필요함 ➡️ 일상어 정의 ➡️ 기술 매핑 ➡️ 장단점 ➡️ 사용법`)에 입각하여 개념을 충분히 전달한 후 다음 단계로 이행한다.
2. **강사 데모**
