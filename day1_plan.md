...
| 모듈 5 | Kubernetes 개요와 로컬 준비 | Kubernetes 활성화, k9s 설치 | kubectl 연결 확인, k9s 화면 확인 |
...
Docker Desktop에서 Kubernetes를 활성화하고 `kubectl` 명령으로 노드와 시스템 Pod를 확인한다. 터미널 UI 기반 Kubernetes 모니터링 도구인 k9s를 설치해 클러스터 상태를 시각적으로 확인하는 습관을 시작한다. Kubernetes 리소스 실습은 2일차 이후로 넘기되, 1일차에는 로컬 클러스터 준비 상태를 확보하는 데 집중한다.
...
### 실습 4. Docker Desktop Kubernetes 활성화 + k9s 설치
- 목표: 로컬 Kubernetes 클러스터를 활성화하고 이후 배포 실습을 위한 기반을 준비한다.
- 체크포인트: Kubernetes 활성화, `kubectl get nodes` 성공, 시스템 Pod 조회 가능, k9s 설치 및 클러스터 화면 표시 확인.
...
- Kubernetes는 1일차에 “배포”까지 밀어 넣기보다, 활성화와 준비 상태 확인까지만 다루는 편이 전체 24시간 커리큘럼 흐름에 안정적이다.
